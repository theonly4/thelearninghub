...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const emailPayload = {
      ...
      to: [adminEmail],
      subject: `New ${contentLabel} Released - ${contentTitle}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #1a1a1a; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #f8fafc; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
            <h2 style="margin: 0 0 16px 0; font-size: 20px; color: #059669;">Hello ${adminName},</h2>
            ...
            <div style="background: white; border-radius: 8px; padding: 16px; margin: 16px 0; border-left: 4px solid #10b981;">
              ...
              <ul style="margin: 0; padding-left: 20px; color: #4b5563;">
                ...
                ${workforceGroup ? `<li><strong>Workforce Group:</strong> ${workforceGroup}</li>` : ''}
                ${trainingYear ? `<li><strong>Training Year:</strong> ${trainingYear}</li>` : ''}
              </ul>
            </div>
          ...
          <div style="text-align: center; margin-bottom: 24px;">
            <a href="${loginUrl}" style="display: inline-block; background: linear-gradient(135deg, #059669 0%, #10b981 100%); color: white; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600; font-size: 16px;">Go to Dashboard</a>
          </div>
